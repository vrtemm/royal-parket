  ...
  var ocfilter = {
    ...
    init: function(options) {
      ...
      $('#ocfilter .scale').each(function(i) {
        ...
        if ((max - min) > 10) {
          ...
          for (; _i < 100; _i += 25) {
            _options['range'][_i + '%'] = Math.ceil(min + ((max - min) / 100 * _i));
          }
		  console.log(_options['range']);
        } else {
          _options['pips'] = {
            mode: 'count',
            values: 3,
    ...
    update: function(scrollTarget) {
      ...
      $.get('index.php?route=module/ocfilter/callback', data, function(json) {
        $('.oc-price-range input').removeAttr('disabled');
		/* Start update */
        ...
        if (json.total === 0) {
		  ...
		  $('#ocfilter-button2 button').removeAttr('onclick').addClass('disabled').text('Ok');
		  ...
		  $('#ocfilter-button2 button').attr('onclick', 'location = \'' + json.href + '\'').removeClass('disabled').attr('disabled', false).text('Ok');
          $('#ocfilter .scale').removeAttr('disabled');
        }
    ...
    preload: function() {
	  ...
	  $('#ocfilter-button2 button').button('loading');
