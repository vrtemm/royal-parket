...
<div class="panel panel-default iblog-panel">
	<div class="panel-heading">Новости, Статьи, Обзоры</div>
	<div class="panel-content iblog-box-content">
        <?php if (!empty($posts)) { ?>
			<ul class="iblog-box-post">
				<?php foreach ($posts as $post) { ?>
					<li>
						<div class="iblog-post-image">
							<a href="<?php echo $post['href']; ?>"><img src="<?php echo $post['image']; ?>" class="img-responsive" alt="<?php echo $post['title']; ?>" /></a>
						</div>
		...
		<?php } ?>
		<a class="sell-all" href="<?php echo $blog; ?>"><span>Все статьи</span> <i class="fa fa-chevron-right"></i></a>
	</div>
</div>
