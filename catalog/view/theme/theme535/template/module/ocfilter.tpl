...
<?php if ($options || $show_price) { ?>
	<div class="box ocfilter panel-default foc-c" id="ocfilter">
		
		<div class="filter-heading box-heading">Выбрать по параметрам:</div>
		
		<div class="hidden" id="ocfilter-button">
			<button class="btn btn-primary disabled" data-loading-text="<i class='fa fa-refresh fa-spin'></i> Загрузка.."></button>
		</div>
		<div class="box-content" id="ocfilter-content">
			...
			<?php if ($show_price) { # Price filtering ?>
				<div class="ocfilter-option" data-toggle="popover-price">
					...
					<div class="option-values option-price-values">
						
						<div class="oc-price-range clearfix"> 
							<div class="oc-flex">
							<div class="oc-form-group"> 
								<input name="price[min]" value="<?php echo $min_price_get; ?>" type="text" class="form-control input-sm" id="min-price-value" /> 
							</div> 
							<div class="oc-form-group oc-splite">-</div> 
							<div class="oc-form-group"> 
								<input name="price[max]" value="<?php echo $max_price_get; ?>" type="text" class="form-control input-sm" id="max-price-value" /> 
							</div>
							<div class="oc-form-group oc-symbol">грн.</div> 
							</div>
							<div></div>
							<div id="ocfilter-button2">
								<button class="btn btn-primary disabled" data-loading-text="<i class='fa fa-refresh fa-spin'></i>">OK</button>
							</div>
						</div> 
		...
		<script type="text/javascript"><!--
			$(function() {
				...
				if($(window).width() < 768) {
					$('#ocfilter').prependTo('#content');
					options['mobile'] = true;
					$('#ocfilter.box .box-heading').on('click', function(){
						$('#ocfilter-content').slideToggle();
					});
				}
