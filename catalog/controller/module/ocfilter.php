<?php
	...
	class ControllerModuleOCFilter extends Controller {
		public $registry;
		private $data = array();
		private $ex = array(34,89,90,84,80,85,83);
		private $ex_f = array(30007);
		...
		public function __construct($registry) {
			...
			if(in_array($this->category_id, $this->ex)) {
				$results = array();
				} else {
			...
			if(isset($options_get) && is_array($options_get) && (count($options_get) == 1) && (count(end($options_get)) == 1)) {
				
				if (!$this->page_info) {
					$this->load->model('catalog/category');
					//print_r($value_keywords);
					//print_r($option_keywords);
					$g = true;
					
					foreach($options_get as $k => $v) {
						if(in_array($k, $this->ex_f)) {
							$g = false;
							$this->document->setNoindex(true);
						}
						if(isset($value_keywords[$k.'-'.end($v)])) {
							$uk = $option_keywords[$k];
							$uv = $value_keywords[$k.'-'.end($v)];
							if($k == 'm') {
								$ukn = 'производитель';
								$uvn = $this->db->query("SELECT `name` FROM `" . DB_PREFIX . "manufacturer` WHERE `manufacturer_id` = '" . end($v) . "'");
								$keyseo = 'm-' . $uv;
								$keypar = $uv;
							} else {
								$ukn = $this->db->query("SELECT `name` FROM `" . DB_PREFIX . "ocfilter_option_description` WHERE `option_id` = '" . $k . "'");
								if(isset($ukn->row['name'])) {
									$ukn = rtrim(trim(mb_strtolower($ukn->row['name'])), ':');
								} else {
									$ukn = false;
								}
								
								$uvn = $this->db->query("SELECT `name` FROM `" . DB_PREFIX . "ocfilter_option_value_description` WHERE `value_id` = '" . end($v) . "'");
					...
					if($g && $cname && $ukn && isset($uvn->row['name']) && $uvn->row['name'] && $uk && $uv) {
						
						
						$check = $this->db->query("SELECT ocfilter_page_id FROM `" . DB_PREFIX . "ocfilter_page` WHERE `category_id`='" . $this->category_id . "' AND `ocfilter_params`='" . $uk . '/' . $uv . "'");
						if($check->num_rows == 0) {
							$title = $cname . ': ' . $ukn . ' ' . $uvn->row['name'] . ' купить в Одессе, Киеве - Royal Parquet';
							$keyword = '';
							$description = '【 ' . $cname . ' 】  ' . $cname . ': ' . $ukn . ' ' . $uvn->row['name'] . ' недорого купить в интернет-магазине Royal-Parquet.ua ➜ ☝️ Официальный представитель ⭐ Большой ассортимент ⚡ Доступные цены ✅Акции и Скидки. ☎ (097) 520-64-61.';
							$h1 = $cname .": " . $ukn . " " . $uvn->row['name'];
							
							$this->db->query("INSERT INTO `" . DB_PREFIX . "ocfilter_page` SET 
							`category_id`='" . $this->category_id . "',
							`ocfilter_params`='" . $keypar . "',
							`meta_title`= '" . $title . "',
		...
		public function decode() {
			...
			foreach ($keywords as $key => $keyword) {
				$page_info = $this->cache->get('ocfilter.page.keyword.' . md5($keyword.$category_id));
				
				if (false === $page_info) {
					$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "ocfilter_page WHERE category_id = '" . $category_id . "' AND status = '1' AND keyword = '" . $this->db->escape($keyword) . "' LIMIT 1");
					
					if ($query->num_rows) {
						$page_info = $query->row;
						
						$this->cache->set('ocfilter.page.keyword.' . md5($keyword.$category_id), $page_info);
					}
				}
